/****************************************************************************
 * arch/risc-v/src/sg2000/sg2000_serial.c
 *
...
#include "hardware/sg2000_uart.h"
...
static int  sg2000_setup(struct uart_dev_s *dev);
static void sg2000_shutdown(struct uart_dev_s *dev);
static int  sg2000_attach(struct uart_dev_s *dev);
static void sg2000_detach(struct uart_dev_s *dev);
static int  sg2000_ioctl(struct file *filep, int cmd, unsigned long arg);
static int  sg2000_receive(struct uart_dev_s *dev, unsigned int *status);
static void sg2000_rxint(struct uart_dev_s *dev, bool enable);
static bool sg2000_rxavailable(struct uart_dev_s *dev);
static void sg2000_send(struct uart_dev_s *dev, int ch);
static void sg2000_txint(struct uart_dev_s *dev, bool enable);
static bool sg2000_txready(struct uart_dev_s *dev);
static bool sg2000_txempty(struct uart_dev_s *dev);
...
{
  .setup       = sg2000_setup,
  .shutdown    = sg2000_shutdown,
  .attach      = sg2000_attach,
  .detach      = sg2000_detach,
  .ioctl       = sg2000_ioctl,
  .receive     = sg2000_receive,
  .rxint       = sg2000_rxint,
  .rxavailable = sg2000_rxavailable,
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  .rxflowcontrol = NULL,
#endif
  .send    = sg2000_send,
  .txint   = sg2000_txint,
  .txready = sg2000_txready,
  .txempty = sg2000_txempty,
};
...
static struct sg2000_uart_s g_uart3priv =
{
  .irq = SG2000_IRQ_UART3,
  .config =
    {
      .idx       = 3,
...
static int __uart_interrupt(int irq, void *context, void *arg)
{
  ...
  struct sg2000_uart_s *priv = dev->priv;
  uint8_t uart_idx = priv->config.idx;
  uint32_t int_status;
  uint32_t int_mask;

  int_status = getreg32(SG2000_UART_INT_STS(uart_idx));
  int_mask = getreg32(SG2000_UART_INT_MASK(uart_idx));
  ...
  if ((int_status & UART_INT_STS_URX_END_INT) &&
      !(int_mask & UART_INT_MASK_CR_URX_END_MASK))
    {
      putreg32(UART_INT_CLEAR_CR_URX_END_CLR,
               SG2000_UART_INT_CLEAR(uart_idx));
...
static void sg2000_uart_configure(const struct uart_config_s *config)
{
...
static int sg2000_setup(struct uart_dev_s *dev)
{
  struct sg2000_uart_s *priv = (struct sg2000_uart_s *)dev->priv;

  sg2000_uart_configure(&priv->config);
  return OK;
}

/****************************************************************************
 * Name: sg2000_shutdown
 *
 * Description:
...
static void sg2000_shutdown(struct uart_dev_s *dev)
{
  ...
  modifyreg32(SG2000_UART_UTX_CONFIG(uart_idx), UART_UTX_CONFIG_CR_EN, 0);
  modifyreg32(SG2000_UART_URX_CONFIG(uart_idx), UART_URX_CONFIG_CR_EN, 0);
}
          ...
          if (ret == OK)
            {
              ...
              tmp_val = getreg32(SG2000_UART_INT_MASK(config.idx));
              sg2000_uart_configure(&config);
              putreg32(tmp_val, SG2000_UART_INT_MASK(config.idx));
            }
        }
      while (0);
      break;
...
static int sg2000_receive(struct uart_dev_s *dev, unsigned int *status)
{
  ...
  if (getreg32(SG2000_UART_FIFO_CONFIG_1(uart_idx)) &
      UART_FIFO_CONFIG_1_RX_CNT_MASK)
    {
      rxdata = getreg32(SG2000_UART_FIFO_RDATA(uart_idx)) &
  ...
  if (enable)
    {
      ...
      int_mask &= ~(UART_INT_MASK_CR_URX_FIFO_MASK);
      int_mask &= ~(UART_INT_MASK_CR_URX_END_MASK);
      putreg32(int_mask, SG2000_UART_INT_MASK(uart_idx));
#endif
    }
  else
    {
...
static bool sg2000_rxavailable(struct uart_dev_s *dev)
{
  ...
  return (getreg32(SG2000_UART_FIFO_CONFIG_1(uart_idx)) &
...
static void sg2000_send(struct uart_dev_s *dev, int ch)
{
  ...
  while ((getreg32(SG2000_UART_FIFO_CONFIG_1(uart_idx)) &
         UART_FIFO_CONFIG_1_TX_CNT_MASK) == 0);

  putreg32(ch, SG2000_UART_FIFO_WDATA(uart_idx));
}
...
void sg2000_earlyserialinit(void)
{
  /* Configuration whichever one is the console */

  CONSOLE_DEV.isconsole = true;
  sg2000_setup(&CONSOLE_DEV);
}

/****************************************************************************
 * Name: sg2000_serialinit
 *
 * Description:
